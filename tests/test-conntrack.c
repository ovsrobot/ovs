    ...
    for (i = 0; i < n_packets_per_conn; i++) {
        ...
        for (j = 0; j < n_conns; j++) {
            ...
            if (i == 0) {
                ...
                /* This is a forward ACK. the existing states are fine. But the
                ...
                /* This is the last ACK of the connection. We must not match it
                 * afterwards. */
            } else if (i % 2 == 1) {
                /* Data. */
                tcp_flags |= TCP_PSH;
...
ct_thread_tcp_main(void *aux_)
{
     ...
     * the same zone at the same time and thereby move in lockstep. That would
     * be unrealistic in real cases. We therefore distribute them as much as
        ...
        for (j = 0; j < aux->n_conns; j++) {
            ...
            if (packet_offset == 0) {
                ...
                if (pkt_batches[j]->packets[0]->md.ct_mark == 0) {
                    ...
                    aux->sleep_counter++;
                    j--;
                    // Each sleep is 1ms, if we slept for a total of 100s we
                    // assume something is broken and we will never get out of
                    // this anymore.
                    ovs_assert(aux->sleep_counter < (100 * 1000));
                    continue;
                }
...
test_benchmark_tcp(struct ovs_cmdl_context *ctx)
{
    ...
    for (i = 0; i < n_threads; i++) {
        ...
        threads[i].thread = ovs_thread_create("ct_thread", ct_thread_tcp_main,
                                              &threads[i]);
    }
    /* Starts the work inside the threads */
    ovs_barrier_block(&barrier);
    ...
    if (lookup_errors) {
        printf("total conntrack lookup errors. Probably the connection expired"
...
static const struct ovs_cmdl_command commands[] = {
    ...
    {"benchmark-tcp", "n_threads n_conns n_zones n_data_pkts iterations", 5, 5,
