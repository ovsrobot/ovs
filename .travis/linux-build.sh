...
function install_dpdk()
{
    ...
    if [ "$DPDK_SHARED" ]; then
        ...
        export LD_LIBRARY_PATH=$(pwd)/dpdk-dir/build/lib/$DPDK_LIB/:$LD_LIBRARY_PATH
    else
        EXTRA_OPTS="$EXTRA_OPTS --with-dpdk=static"
    fi

    # Export the following path for pkg-config to find the .pc file.
    export PKG_CONFIG_PATH=$(pwd)/dpdk-dir/build/lib/$DPDK_LIB/pkgconfig/:$PKG_CONFIG_PATH
