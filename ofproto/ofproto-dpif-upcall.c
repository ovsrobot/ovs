...
recv_upcalls(struct handler *handler)
{
    ...
    while (n_upcalls < UPCALL_MAX_BATCH) {
        ...
        /* Fill flow with key_as_flow as upcall_receive needs packet flow info */
        *flow = *key_as_flow;
...
should_install_flow(struct udpif *udpif, struct upcall *upcall)
{
    ...
    if (upcall->key_len){
        if (!flow_equal_except(upcall->key_as_flow, upcall->flow, &upcall->wc)){
            VLOG_INFO_RL(&rl, "upcall: inconsistent on datapath key and vswitchd"
                         " extracted key. Datapath flow will not be installed\n"
