    ...
    HMAP_FOR_EACH_SAFE (pr_op, hmap_node, &bond->pr_rule_ops) {
        ...
        case ADD:
            ...
            if (error) {
                ...
                VLOG_ERR("bond %s: failed to add post recirculation flow %s",
                         bond->name, err_s);
                free(err_s);
            }
            break;
        ...
        case DEL:
            ...
            if (error) {
                ...
                VLOG_ERR("bond %s: failed to remove post recirculation flow "
                         "%s", bond->name, err_s);
                free(err_s);
            }
...
bond_reconfigure(struct bond *bond, const struct bond_settings *s)
{
    ...
    if (bond->use_lb_output_action != s->use_lb_output_action) {
        if (s->use_lb_output_action &&
            ...
            VLOG_WARN("bond %s: Datapath does not support 'lb_output' action, "
                      "disabled", bond->name);
        } else {
...
bond_rebalance(struct bond *bond)
{
    ...
    while (!ovs_list_is_short(&bals)) {
        ...
        VLOG_INFO("bond %s: shifted %"PRIuSIZE" hashes totaling %"PRIu64
...
bond_enable_member(struct bond_member *member, bool enable)
{
    ...
    if (enable != member->enabled) {
        ...
        VLOG_INFO("bond %s: member %s %s", bond->name, member->name,
...
bond_link_status_update(struct bond_member *member)
{
    ...
    if ((up == member->enabled) != (member->delay_expires == LLONG_MAX)) {
        ...
        VLOG_INFO_RL(&rl, "bond %s: member %s %s %s", bond->name,
                     member->name, msg, up ? "up" : "down");
        if (up == member->enabled) {
            member->delay_expires = LLONG_MAX;
            VLOG_INFO_RL(&rl, "bond %s: member %s will not be %s",
                         bond->name, member->name,
                         up ? "disabled" : "enabled");
        } else {
            ...
            if (delay) {
                VLOG_INFO_RL(&rl, "bond %s: member %s will be %s"
...
error:
    if (VLOG_IS_DBG_ENABLED()) {
        ...
        VLOG_DBG_RL(&rl, "bond %s: cannot choose output member "
