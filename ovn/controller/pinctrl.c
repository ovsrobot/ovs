                        ...
                        const struct sbrec_ip_multicast *ip_mcast)
{
    ...
    cfg->enabled = ip_mcast->enabled;
    cfg->querier_enabled = (ip_mcast->enabled && ip_mcast->querier);

    cfg->table_size = ip_mcast->table_size;
    cfg->idle_time_s = ip_mcast->idle_timeout;
    cfg->query_interval_s = ip_mcast->query_interval;
    cfg->query_max_resp_s = ip_mcast->query_max_resp;
    ...
    if (cfg->querier_enabled) {
        ...
        if (!ip_mcast->eth_src ||
                !eth_addr_from_string(ip_mcast->eth_src,
                                      &cfg->query_eth_src)) {
            VLOG_WARN("IGMP Querier enabled with invalid ETH source address");
        ...
        if (!ip_mcast->ip4_src ||
            ...
            VLOG_WARN("IGMP Querier enabled with invalid IPv4 source address");
            /* Failed to parse the IPv4 source address. Disable the querier. */
            cfg->querier_enabled = false;
        }
                      ...
                      long long int current_time)
{
    ...
    put_load(OVN_MULTICAST_FLOOD_TUNNEL_KEY, MFF_LOG_OUTPORT, 0, 32, &ofpacts);
