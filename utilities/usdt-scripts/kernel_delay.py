...
TRACEPOINT_PROBE(raw_syscalls, sys_enter) {
    ...
    struct long_poll_data_t *val, zero = {};
    ...
    val = long_poll_data.lookup_or_try_init(&key, &zero);
    if (val) {
        u64 delta = t - *start_ns;
        val->count++;
        if (val->avg_ns > 0)
            val->avg_ns = (val->avg_ns + delta) / 2;
        else
            val->avg_ns = delta;
        if (delta > val->max_ns)
            val->max_ns = delta;
        else if (delta < val->min_ns || val->min_ns == 0)
            val->min_ns = delta;
    }
    ...
    for thread in threads:
        ...
        for k, v in filter(lambda t: t[0].tid == thread,
            ...
            print("{}{:10}  {:16,}  {:16,}  {:16,}".format(
                indent, v.count, v.avg_ns, v.min_ns, v.max_ns))
            break
