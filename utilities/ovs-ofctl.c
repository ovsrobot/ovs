...
/* "compose-packet [--pcap] [--hexified] [--bad-csum] FLOW [L7]": Converts the
 ...
 *
 * With --hexified, prints the packet as a single bare hex string with no
 * spaces or offsets, so that you can pass the result directly to e.g.
 * "ovs-appctl netdev-dummy/receive vif $(ovs-ofctl compose-packet --hexified
 * FLOW)"
 *
