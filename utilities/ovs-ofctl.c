...
/* --len: Makes "compose-packet" extend the generated packet. */
 ...
 *
 * With --len, produces a packet that is extended to the given length.
 *
...
ofctl_compose_packet(struct ovs_cmdl_context *ctx)
{
    ...
    if (opt_len) {
        ...
        if (opt_len > p_len) {
            packet_expand(&p, &flow1, opt_len);
        } else if (opt_len < p_len) {
            ovs_fatal(1, "--len can not be used to truncate the packet");
        }
    }
