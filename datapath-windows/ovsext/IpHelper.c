...
OvsDumpIPEntry(PMIB_UNICASTIPADDRESS_ROW ipRow)
{
    ...
    if (ipRow->Address.si_family == AF_INET) {
        ...
        pAddr = (struct in6_addr *)&(ipRow->Address.Ipv6.sin6_addr);
        if (!RtlIpv6AddressToStringA(pAddr, wszAddr)) {
            OVS_LOG_INFO("Unicast Ipv6 Address got failed\n");
        } else {
            OVS_LOG_INFO("Unicast Ipv6 Address: %s", wszAddr);
        }
    }
}
             ...
             PMIB_IPFORWARD_ROW2 route)
{
    if (destinationAddress->si_family == AF_INET) {
        ...
        OVS_LOG_INFO("NextHop: %d.%d.%d.%d",
                     ipAddr & 0xff, (ipAddr >> 8) & 0xff,
                     (ipAddr >> 16) & 0xff, (ipAddr >> 24) & 0xff);
     } else if (destinationAddress->si_family == AF_INET6) {
         struct in6_addr *pAddr = NULL;
         char wszAddr[256] = {0};

         pAddr = (struct in6_addr *)&(destinationAddress->Ipv6.sin6_addr);
         if (!RtlIpv6AddressToStringA(pAddr, wszAddr)) {
             OVS_LOG_INFO("Destination Ipv6 Address got failed\n");
         } else {
             OVS_LOG_INFO("Destination: %s", wszAddr);
         }

         RtlZeroMemory(wszAddr, sizeof(wszAddr));
         pAddr = (struct in6_addr *)&(sourceAddress->Ipv6.sin6_addr);
         if (!RtlIpv6AddressToStringA(pAddr, wszAddr)) {
             OVS_LOG_INFO("Source Ipv6 Address got failed\n");
         } else {
             OVS_LOG_INFO("Source: %s", wszAddr);
         }

         RtlZeroMemory(wszAddr, sizeof(wszAddr));
         pAddr = (struct in6_addr*)&(route->NextHop.Ipv6.sin6_addr);
         if (!RtlIpv6AddressToStringA(pAddr, wszAddr)) {
             OVS_LOG_INFO("NextHop Ipv6 Address got failed\n");
         } else {
             OVS_LOG_INFO("NextHop: %s", wszAddr);
         }
     }
}
...
OvsDumpIPNeigh(PMIB_IPNET_ROW2 ipNeigh)
{
    ...
    if (ipNeigh->Address.si_family == AF_INET) {
        UINT32 ipAddr = ipNeigh->Address.Ipv4.sin_addr.s_addr;

        OVS_LOG_INFO("Neigh: %d.%d.%d.%d",
                     ipAddr & 0xff, (ipAddr >> 8) & 0xff,
                     (ipAddr >> 16) & 0xff, (ipAddr >> 24) & 0xff);
    } else if (ipNeigh->Address.si_family == AF_INET6) {
        struct in6_addr *pAddr = NULL;
        char wszAddr[256] = {0};
        pAddr = (struct in6_addr *)&(ipNeigh->Address.Ipv6.sin6_addr);
        if (!RtlIpv6AddressToStringA(pAddr, wszAddr)) {
            OVS_LOG_INFO("Neigh Ipv6 Address got failed\n");
        } else {
            OVS_LOG_INFO("Neigh: %s", wszAddr);
        }
    }
    ...
    if (status != STATUS_SUCCESS) {
        if (ipNeigh->Address.si_family == AF_INET) {
           ...
           if (!RtlIpv6AddressToStringA(pAddr, wszAddr)) {
               ...
               OVS_LOG_INFO("Fail to get ARP entry: %s, status: %x",
                             wszAddr, status);
           }
       }
       return status;
    ...
    if (status != STATUS_SUCCESS) {
        if (ipNeigh->Address.si_family == AF_INET) {
            ...
            if (!RtlIpv6AddressToStringA(pAddr, wszAddr)) {
                ...
                OVS_LOG_INFO("Fail to resolve ARP entry: %s, status: %x",
                             wszAddr, status);
            }
        }
        return status;
        ...
        LIST_FORALL_SAFE(head, link, next) {
            ...
            if (instance->isIpConfigured &&
                ...
                if (iphAddr.si_family == AF_INET) {
                    ...
                    struct in6_addr *pAddr = NULL;
                    char wszAddr[256] = {0};
                    pAddr = (struct in6_addr *)&(unicastRow->Address.Ipv6.sin6_addr);
                    if (!RtlIpv6AddressToStringA(pAddr, wszAddr)) {
                        OVS_LOG_INFO("Ipv6 Address got failed\n");
                    } else {
                        OVS_LOG_INFO("IP Address: %s is %s", wszAddr,
                                     notificationType == MibAddInstance ?
                                     "added": "modified");
                    }
                }
        ...
        if (found) {
            ...
            if (iphAddr.si_family == AF_INET) {
                ...
                if (!RtlIpv6AddressToStringA(pAddr, wszAddr)) {
                    ...
                    OVS_LOG_INFO("IP Address removed: %s", wszAddr);
                }
            }
        }
...
OvsHandleFwdRequest(POVS_IP_HELPER_REQUEST request)
{
    ...
    if (!OvsIphIsZero(&(request->fwdReq.tunnelKey.src)) &&
        ...
        if (request->fwdReq.tunnelKey.dst.si_family == AF_INET) {
            UINT32 tempAddr = request->fwdReq.tunnelKey.dst.Ipv4.sin_addr.s_addr;
            OVS_LOG_INFO("Fail to get route to %d.%d.%d.%d, status: %x",
                         tempAddr & 0xff, (tempAddr >> 8) & 0xff,
                         (tempAddr >> 16) & 0xff, (tempAddr >> 24) & 0xff, status);
        } else if (request->fwdReq.tunnelKey.dst.si_family == AF_INET6) {
             struct in6_addr *pAddr = NULL;
             char  wszAddr[256] = {0};

             pAddr = (struct in6_addr *)&(request->fwdReq.tunnelKey.dst.Ipv6.sin6_addr);
             if (!RtlIpv6AddressToStringA(pAddr, wszAddr)) {
                 OVS_LOG_INFO("route Ipv6 Address got failed\n");
             } else {
                 OVS_LOG_INFO("Fail to get route to %s, status: %x",
                              wszAddr, status);
             }
        }
        goto fwd_handle_nbl;
    ...
    if (status != STATUS_SUCCESS || instance == NULL) {
        if (request->fwdReq.tunnelKey.dst.si_family == AF_INET) {
            UINT32 tempAddr =
                   request->fwdReq.tunnelKey.dst.Ipv4.sin_addr.s_addr;
            OVS_LOG_INFO("Fail to get route to %d.%d.%d.%d, status: %x",
                         tempAddr & 0xff, (tempAddr >> 8) & 0xff,
                         (tempAddr >> 16) & 0xff, (tempAddr >> 24) & 0xff,
                         status);
        } else if (request->fwdReq.tunnelKey.dst.si_family == AF_INET6) {
             struct in6_addr *pAddr = NULL;
             char wszAddr[256] = {0};
             pAddr = (struct in6_addr*)
                     &(request->fwdReq.tunnelKey.dst.Ipv6.sin6_addr);
             if (!RtlIpv6AddressToStringA(pAddr, wszAddr)) {
                 OVS_LOG_INFO("route Ipv6 Address got failed\n");
             } else {
                 OVS_LOG_INFO("Fail to get route to %s, status: %x",
...
fwd_handle_nbl:
    ...
    if (status != STATUS_SUCCESS) {
        ...
        if (iphAddr.si_family == AF_INET) {
            UINT32 ipA = iphAddr.Ipv4.sin_addr.s_addr;
            ...
            struct in6_addr *pAddr = NULL;
            char wszAddr[256] = {0};
            pAddr = (struct in6_addr *)&(iphAddr.Ipv6.sin6_addr);
            if (!RtlIpv6AddressToStringA(pAddr, wszAddr)) {
                OVS_LOG_INFO("dst Ipv6 Address got failed\n");
            } else {
                OVS_LOG_INFO("Fail to handle IP helper request for dst: %s",
                             wszAddr);
            }
        }
    }
