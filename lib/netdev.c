...
COVERAGE_DEFINE(netdev_send_multi);
...
static int
netdev_send_burst(struct netdev *netdev, int qid,
...
static int
netdev_send_multi(struct netdev *netdev, int qid,
                  struct dp_packet_batch *batch, bool concurrent_txq)
{
    ...
    COVERAGE_INC(netdev_send_multi);
    ...
    do {
        ...
        dp_packet_batch_add_array(&smaller_batch, &batch->packets[sent],
                                  count);
        error = netdev_send_burst(netdev, qid, &smaller_batch, concurrent_txq);
        sent += count;
        if (error) {
            for (; sent < batch_cnt; sent++) {
                dp_packet_delete(batch->packets[sent]);
            }
            break;
        }
            ...
            bool concurrent_txq)
{
        ...
        if (!(netdev_flags & NETDEV_TX_OFFLOAD_TCP_TSO)) {
            DP_PACKET_BATCH_FOR_EACH (i, packet, batch) {
                if (dp_packet_get_tso_segsz(packet)) {
                    ...
                    return netdev_send_multi(netdev, qid, batch,
                                             concurrent_txq);
                }
            }
    ...
    return netdev_send_burst(netdev, qid, batch, concurrent_txq);
}
