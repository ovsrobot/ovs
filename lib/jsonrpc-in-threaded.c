...
void jsonrpc_in_threaded_cleanup(struct jsonrpc_in_threaded *tin) {
    ...
    while (tin->jsons_tail != tin->jsons_head) {
        json_destroy(tin->jsons[tin->jsons_tail % JSON_RPC_IN_NUM_PENDING_JSONS]);
        tin->jsons_tail++;
    }
...
static void *jsonrpc_in_threaded_worker(void *tin_raw) {
    ...
    for (;;) {
        ...
        ovs_assert(tin->jsons_head - tin->jsons_tail < JSON_RPC_IN_NUM_PENDING_JSONS);
