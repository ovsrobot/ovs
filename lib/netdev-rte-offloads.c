...
netdev_rte_offloads_validate_flow(const struct match *match)
{
    ...
    /* unsupported L2 */
    if (!is_all_zeros(&match->wc.masks.mpls_lse,
    ...
    if (match->wc.masks.ipv6_label ||
        ...
        !is_all_zeros(&match->wc.masks.ipv6_src, sizeof(struct in6_addr)) ||
        !is_all_zeros(&match->wc.masks.ipv6_dst, sizeof(struct in6_addr)) ||
        !is_all_zeros(&match->wc.masks.ct_ipv6_src, sizeof(struct in6_addr)) ||
        !is_all_zeros(&match->wc.masks.ct_ipv6_dst, sizeof(struct in6_addr)) ||
        !is_all_zeros(&match->wc.masks.nd_target, sizeof(struct in6_addr)) ||
        !is_all_zeros(&match->wc.masks.nsh, sizeof(struct ovs_key_nsh)) ||
        !is_all_zeros(&match->wc.masks.arp_sha, sizeof(struct eth_addr)) ||
        !is_all_zeros(&match->wc.masks.arp_tha, sizeof(struct eth_addr))) {
        goto err;
    }
