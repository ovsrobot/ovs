...
struct ovsdb_cs_row_update {
    struct hmap_node hmap_node; /* Indexed by 'row_uuid'. */
...
struct ovsdb_cs_table_update {
    struct hmap row_updates;    /* Contains "struct ovsdb_cs_row_update *". */
};

struct ovsdb_error *ovsdb_cs_parse_table_updates(
    const struct json *json_table_updates, int version,
    struct shash *parsed_table_updates)
    OVS_WARN_UNUSED_RESULT;
void ovsdb_cs_table_updates_destroy(struct shash *table_updates);

