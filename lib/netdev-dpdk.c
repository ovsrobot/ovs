...
#define NETDEV_DPDK_GLOBAL_MUTEX_NAME dpdk_mutex
...
void *
netdev_dpdk_watchdog(void *params_)
{
    struct netdev_dpdk_watchdog_params *params = params_;
    struct netdev_dpdk_common *common;
...
common_construct(struct netdev *netdev, dpdk_port_t port_no,
                 enum netdev_dpdk_dev_type type, int socket_id)
    OVS_REQUIRES(dpdk_mutex)
{
    ...
    dev->common.sw_stats->tx_retries =
        (dev->common.type == DPDK_DEV_VHOST) ? 0 : UINT64_MAX;
                         ...
                         const struct smap *args)
{
    ...
    if (flags && dev->common.type != DPDK_DEV_ETH) {
...
netdev_dpdk_get_status(const struct netdev *netdev, struct smap *args)
{
    ...
    ret = netdev_dpdk_get_eth_dev_status(netdev, &dpdk_mutex, args);
    if (ret) {
        return ret;
    }
...
netdev_dpdk_flow_api_supported(struct netdev *netdev, bool check_only)
{
    ...
    dev = netdev_dpdk_cast(netdev);
    ovs_mutex_lock(&dev->common.mutex);
    if (dev->common.type == DPDK_DEV_ETH) {
