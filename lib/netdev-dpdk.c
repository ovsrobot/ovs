...
const struct rte_eth_conf dpdk_default_eth_conf = {
    .rxmode = {
        .offloads = 0,
    },
    .rx_adv_conf = {
...
static struct ovs_mutex dpdk_mp_mutex OVS_ACQ_AFTER(dpdk_common_mutex)
...
static bool
netdev_dpdk_is_eth(const struct netdev *netdev)
{
...
dpdk_calculate_mbufs(struct netdev_dpdk *dev, int mtu)
{
    ...
    if (!per_port_memory) {
        ...
        n_mbufs =
            dev->common.requested_n_rxq * dev->common.requested_rxq_size
            + dev->common.requested_n_txq * dev->common.requested_txq_size
            + MIN(RTE_MAX_LCORE, dev->common.requested_n_rxq)
              * NETDEV_MAX_BURST
            + MIN_NB_MBUF;
    }
...
dpdk_watchdog(void *dummy OVS_UNUSED)
{
    ...
    for (;;) {
        ...
        LIST_FOR_EACH (dev, common.list_node, &dpdk_list) {
            ...
            if (netdev_dpdk_is_eth(&dev->common.up)) {
...
dpdk_eth_dev_port_config(struct netdev_dpdk *dev,
                         ...
                         int n_rxq, int n_txq)
{
    struct rte_eth_conf conf = dpdk_default_eth_conf;
    uint16_t conf_mtu;
    int diag = 0;
    int i;
...
static struct netdev_dpdk_tx_queue *
    ...
    OVS_REQUIRES(dpdk_common_mutex)
{
    ...
    dev->common.sw_stats->tx_retries =
        netdev_dpdk_is_eth(netdev) ? UINT64_MAX : 0;
                         ...
                         const struct smap *args)
{
    ...
    if (flags && !netdev_dpdk_is_eth(netdev)) {
