...
static void
dpdk_rx_steer_set_config(struct netdev *netdev, struct netdev_dpdk *dev,
                       ...
                       char **errp)
{
    ...
    /* needs to be called before dpdk_set_rxq_config() */
    dpdk_rx_steer_set_config(netdev, dev, args, errp);
...
dpdk_rx_steer_configure(struct netdev_dpdk *dev)
{
    ...
    if (dev->rx_steer_flows_num) {
        ...
        if (dev->up.n_rxq == 2) {
            ...
            VLOG_INFO("%s: rx-steering: applied rss on rx queue 0-%u",
