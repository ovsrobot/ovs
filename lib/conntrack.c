...
conn_clean__(struct conntrack *ct, struct conn *conn)
{
    ...
    bucket = &ct->buckets[hash_to_bucket(hash)];
...
{
    unsigned p1 = hash_to_bucket(h1),
        p2 = hash_to_bucket(h2);
               ...
               enum ct_alg_ctl_type ct_alg_ctl, uint32_t tp_id)
{
    ...
    if (commit) {
        ...
        cmap_insert(&ct->buckets[hash_to_bucket(ctx->hash)].conns,
                    &fwd_key_node->cm_node, ctx->hash);
        if (nat_action_info) {
            rev_key_node->key_hash = rev_hash;
            cmap_insert(&ct->buckets[hash_to_bucket(rev_hash)].conns,
...
static void
find_addr(struct conn_key *key, union ct_addr *min,
 ...
 * If successful, this function returns with the locks
 * acquired. */
static bool
nat_get_unique_tuple_lock(struct conntrack *ct, struct conn *conn,
                          const struct nat_action_info_t *nat_info,
