...
static bool
pkt_validate_and_set_new_ct_state(struct dp_packet *pkt,
                                  struct conn_lookup_ctx *ctx,
...
#define CONN_SWEEP_BUF_SIZE 100

static void
ct_sweep_buf(struct conntrack *ct, uint16_t zone,
...
static void
expectation_remove(struct conntrack *ct,
                   const struct conn_key *key, uint32_t basis)
    OVS_REQUIRES(ct->resources_lock)
{
    struct alg_exp_node *alg_exp_node;
    ...
    CMAP_FOR_EACH_WITH_HASH (alg_exp_node, node, hash,
        ...
        if (!conn_key_cmp(&alg_exp_node->key, key)) {
            cmap_remove(&ct->alg_expectations, &alg_exp_node->node, hash);
            break;
        }
    }
}
...
static struct alg_exp_node *
expectation_ref_lookup_unique(const struct conntrack *ct,
...
static void
expectation_ref_create(struct conntrack *ct,
                       struct alg_exp_node *alg_exp_node,
                       uint32_t basis)
    OVS_REQUIRES(ct->resources_lock)
{
    if (!expectation_ref_lookup_unique(ct,
                                       &alg_exp_node->parent_key,
                                       &alg_exp_node->key, basis)) {
        hindex_insert(&ct->alg_expectation_refs, &alg_exp_node->node_ref,
    ...
    OVS_REQUIRES(ct->resources_lock)
{
    ...
    HINDEX_FOR_EACH_SAFE (node, node_ref, &ct->alg_expectation_refs) {
        expectation_remove(ct, &node->key,
                           ct->hash_basis);
                   ...
                   bool skip_nat)
{
    ...
    expectation_ref_create(ct, alg_exp_node,
                           ct->hash_basis);
    ovs_mutex_unlock(&ct->resources_lock);
}
