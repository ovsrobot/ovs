    ...
    if (neigh) {
        ...
        if (state == TNL_NEIGH_REACHABLE) {
            ...
            *mac = neigh->mac;
            ...
            /* Test and claim the same deadline so that only one thread
             * retransmits after it expires. */
            atomic_read_explicit(&neigh->expires, &expires,
                                 memory_order_acquire);
            if (expires > now) {
                res = EINPROGRESS;
            } else if (insert_partial && retrans_time) {
                long long next = now + retrans_time;
...
{
    const struct tnl_neigh_entry_ref *a = a_;
    const struct tnl_neigh_entry_ref *b = b_;

    return a->used > b->used ? 1 : a->used < b->used ? -1 : 0;
}
...
{
    struct tnl_neigh_entry_ref *entries;
    struct tnl_neigh_entry *neigh;
    size_t n_stale = 0;
    size_t i = 0;
    ...
    CMAP_FOR_EACH (neigh, cmap_node, &table) {
        if (tnl_neigh_get_state(neigh) == TNL_NEIGH_STALE) {
            entries[i].neigh = neigh;
            atomic_read_explicit(&neigh->used, &entries[i].used,
                                 memory_order_acquire);
            i++;
        }
    }
    ...
    for (i = 0; i < n_stale - NEIGH_ENTRY_MAX_STALE; i++) {
        tnl_neigh_delete(entries[i].neigh);
    }
    free(entries);
