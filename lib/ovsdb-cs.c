                              ...
                              const struct ovsdb_cs_event *event)
{
    ...
    struct shash table_updates;
    struct ovsdb_error *error = ovsdb_cs_parse_table_updates(
        update->table_updates, update->version, &table_updates);
    if (error) {
        log_parse_update_error(error);
        return;
    }
    ...
    const struct ovsdb_cs_table_update *table_update = shash_find_data(
        &table_updates, "Database");
    if (table_update) {
        const struct ovsdb_cs_row_update *ru;
        HMAP_FOR_EACH (ru, hmap_node, &table_update->row_updates) {
            struct server_row *row
    ...
    ovsdb_cs_table_updates_destroy(&table_updates);
}
                            ...
                            struct ovsdb_cs_table_update *out)
{
    ...
    SHASH_FOR_EACH (node, json_object(in)) {
        ...
        const struct json *in_row_update = node->data;
        struct ovsdb_cs_row_update *out_row_update
            = xmalloc(sizeof *out_row_update);
        hmap_insert(&out->row_updates, &out_row_update->hmap_node,
                    uuid_hash(&row_uuid));
        out_row_update->row_uuid = row_uuid;
        out_row_update->columns = NULL;

        struct ovsdb_error *error = ovsdb_cs_parse_row_update(
            table_name, in_row_update, version, out_row_update);
        if (error) {
            return error;
        }
    }
...
ovsdb_cs_parse_table_updates(const struct json *in, int version,
                             struct shash *out)
{
    const char *suffix = version > 1 ? "2" : "";
    ...
    SHASH_FOR_EACH (node, json_object(in)) {
        const char *table_name = node->name;
        const struct json *in_table_update = node->data;

        struct ovsdb_cs_table_update *out_table_update
            = xmalloc(sizeof *out_table_update);
        hmap_init(&out_table_update->row_updates);
        shash_add_assert(out, table_name, out_table_update);

        struct ovsdb_error *error = ovsdb_cs_parse_table_update(
            table_name, in_table_update, version, out_table_update);
        if (error) {
            ovsdb_cs_table_updates_destroy(out);
            shash_init(out);
            return error;
        }
    }
...
void
ovsdb_cs_table_updates_destroy(struct shash *table_updates)
{
    struct shash_node *node, *next_node;
    SHASH_FOR_EACH_SAFE (node, next_node, table_updates) {
        struct ovsdb_cs_table_update *tu = node->data;
        struct ovsdb_cs_row_update *ru, *next_ru;
        HMAP_FOR_EACH_SAFE (ru, next_ru, hmap_node, &tu->row_updates) {
            hmap_remove(&tu->row_updates, &ru->hmap_node);
            free(ru);
        }
        hmap_destroy(&tu->row_updates);
        free(tu);

        shash_delete(table_updates, node);
    }
    shash_destroy(table_updates);
}
