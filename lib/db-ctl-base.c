             ...
             struct shash *filters)
{
    ...
    SSET_FOR_EACH (item, &initialized_filters) {
        ...
        if (ptr && item[len - 1] == ')') {
            ...
            values = xstrdup(item);
            table_name = xstrdup(top_table->name);
            sset_from_delimited_string(&parsed_values, values, "|");

            struct sset *value_set =
                shash_find_data(filters, top_table->name);
            if (!value_set) {
                value_set = xzalloc(sizeof *value_set);
                sset_init(value_set);
                sset_add(value_set, values);
                shash_add(filters, top_table->name, value_set);
            } else {
                sset_add(value_set, values);
            }
        } else {
