...
ovsdb_idl_parse_update__(struct ovsdb_idl *idl,
                         const struct shash *table_updates)
{
    const struct shash_node *tables_node;
    SHASH_FOR_EACH (tables_node, table_updates) {
        const char *table_name = tables_node->name;
        const struct ovsdb_cs_table_update *table_update = tables_node->data;

        struct ovsdb_idl_table *table = shash_find_data(&idl->table_by_name,
                                                        table_name);
        if (!table) {
            return ovsdb_syntax_error(
                NULL, NULL, "update to unknown table \"%s\"", table_name);
        }

        const struct ovsdb_cs_row_update *row_update;
        HMAP_FOR_EACH (row_update, hmap_node, &table_update->row_updates) {
            switch (ovsdb_idl_process_update(table, row_update)) {
