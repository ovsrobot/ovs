                  ...
                  struct in6_addr *out_src, struct in6_addr *gw)
{
    ...
    if (ip6_src) {
        ...
        if (is_ipv4) {
            flow = (struct flow) {.nw_dst = in6_addr_get_mapped_ipv4(ip6_src),
                          ...
                          uint8_t plen)
{
    ...
    if (IN6_IS_ADDR_V4MAPPED(ip6_dst)) {
        ...
        match_set_ipv6_dst_masked(match, ip6_dst, &mask);
    }
}
                    ...
                    const struct in6_addr *ip6_src)
{
    ...
    if (match.flow.dl_type == htons(ETH_TYPE_IPV6)) {
        ...
        in6_addr_set_mapped_ipv4(&p->nw_addr, match.flow.nw_dst);
    }
    p->plen = plen;
    p->user = user;
    p->priority = priority;
...
ovs_router_lookup_cmd(struct unixctl_conn *conn, int argc,
                      const char *argv[], void *aux OVS_UNUSED)
{
    struct in6_addr gw, out_src, src6;
    struct in6_addr *in_src = NULL;
    char src6_s[IPV6_SCAN_LEN + 1];
    char iface[IFNAMSIZ];
    struct in6_addr ip6;
    unsigned int plen;
    ...
    if (ovs_router_lookup(mark, &ip6, in_src, iface, &out_src, &gw)) {
        struct ds ds = DS_EMPTY_INITIALIZER;

        ds_put_format(&ds, "src ");
        ipv6_format_mapped(&out_src, &ds);
        ds_put_format(&ds, "\ngateway ");
