                          ...
                          const struct ovsdb_type *type)
{
    ...
    for (oi = ni = 0; oi < old_datum->n && ni < new_datum->n;) {
        int c = ovsdb_atom_compare_3way(&old_datum->keys[oi], &new_datum->keys[ni],
                                        type->key.type);
        if (c < 0) {
            ...
            if (type->value.type != OVSDB_TYPE_VOID &&
                ovsdb_atom_compare_3way(&old_datum->values[oi], &new_datum->values[ni],
                                        type->value.type)) {
                ovsdb_datum_add_unsafe(removed, &old_datum->keys[oi],
                                       &old_datum->values[oi], type, NULL);
                ovsdb_datum_add_unsafe(added, &new_datum->keys[ni], &new_datum->values[ni],
                                       type, NULL);
            }
            oi++; ni++;
        }
