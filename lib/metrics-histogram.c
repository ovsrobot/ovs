...
{
    struct metrics_histogram *hist = metrics_node_cast(node);
    struct histogram *histogram = hist->get(visitor->it);
    ...
    metrics_visitor_labels_push(visitor, &label_le, 1);

    for (i = 0; i < HISTOGRAM_N_BINS - 1; i++) {
        snprintf(le_value, sizeof(le_value), "%"PRIu32, histogram->wall[i]);
        metrics_header_add_line(hdr, "_buckets", visitor, values[i]);
    }
    /* +Inf bucket */
    ...
    /* Sum field */
    metrics_header_add_line(hdr, "_sum", visitor, values[i++]);

    /* Count field */
    metrics_header_add_line(hdr, "_count", visitor, values[i++]);
}
