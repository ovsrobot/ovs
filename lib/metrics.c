...
metrics_register(struct metrics_node *node)
{
    ...
    ovs_assert("Only register 'METRICS_ENTRIES' or 'METRICS_HISTOGRAM' "
               ...
               (node->type == METRICS_NODE_TYPE_ARRAY ||
...
metrics_tree_size(void)
{
    ...
    metrics_visitor_dfs(&visitor, METRICS_ROOT);
    return total_size;
}
                         ...
                         double value)
{
    ...
    hdr = metrics_header_create(aux, ds_cstr(&full_name),
                                entry);
    metrics_header_add_line(hdr, NULL, visitor, value);
...
metrics_values_format(struct ds *s)
{
    ...
    metrics_values_add_entry(&aux, &visitor, &duration,
...
struct metrics_class *metrics_classes[METRICS_N_NODE_TYPE] = {
    ...
    [METRICS_NODE_TYPE_IF] = &metrics_class_default,
    [METRICS_NODE_TYPE_LABEL] = &metrics_class_add_label,
    [METRICS_NODE_TYPE_FOREACH] = &metrics_class_default,
    [METRICS_NODE_TYPE_ARRAY] = &metrics_class_array,
