...
/* IP/IPv6 address and CIDR mask parsing utilities.
 *
 * These are split from packets.c so that basic utilities that are distributed
 * can use them without depending on the full switching datapath library. */
...
char *ipv6_parse_masked(const char *s, struct in6_addr *ip,
                        struct in6_addr *mask)
    OVS_WARN_UNUSED_RESULT;
char *ipv6_parse_cidr(const char *s, struct in6_addr *ip, unsigned int *plen)
    OVS_WARN_UNUSED_RESULT;
char *ipv6_parse_masked_len(const char *s, int *n, struct in6_addr *ip,
                            struct in6_addr *mask)
    OVS_WARN_UNUSED_RESULT;
...
uint32_t ip_csum_pseudoheader(const struct ip_header *);
...
void format_tcp_flags(struct ds *, uint16_t);
const char *tcp_flag_to_string(uint32_t flag);
...
uint32_t ip_csum_pseudoheader6(const struct ovs_16aligned_ip6_hdr *);
ovs_be16 ip_csum_upperlayer6(const struct ovs_16aligned_ip6_hdr *,
                             const void *, uint8_t, uint16_t);
void ip_set_ipv6_flow_label(ovs_16aligned_be32 *flow_label,
                            ovs_be32 flow_key);
void ip_set_ipv6_tc(ovs_16aligned_be32 *flow_label, uint8_t tc);
...
/*
 * Use the same struct for MLD and MLD2, naming members as the
 * defined fields in the corresponding version of the protocol,
 * though they are reserved in the other one.
 */
...
enum packet_type {
    ...
    /* Pseudo PT for decap. */
    PT_USE_NEXT_PROTO = PACKET_TYPE(OFPHTN_ONF, 0xfffe),
