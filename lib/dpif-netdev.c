...
/* Contained by struct tx_bond 'sub_buckets'. */
struct sub_entry {
    odp_port_t sub_id;
...
struct tx_bond {
    ...
    uint32_t bond_id;
    struct sub_entry sub_buckets[BOND_BUCKETS];
};
    ...
    if (cmap_count(&dp->tx_bonds) > 0) {
        ...
        CMAP_FOR_EACH (dp_bond_entry, node, &dp->tx_bonds) {
            ...
            for (int bucket = 0; bucket < BOND_BUCKETS; bucket++) {
                sub_id = odp_to_u32(dp_bond_entry->sub_buckets[bucket].sub_id);
                ds_put_format(&reply,
                              "    bucket %d - sub-interface %"PRIu32"\n",
                              bucket, sub_id);
            }
        }
    }
    ...
    OVS_EXCLUDED(pmd->bond_mutex)
{
    ...
    if (tx) {
        ...
        for (int i = 0; i < BOND_BUCKETS; i++) {
            ...
            atomic_read_relaxed(&tx->sub_buckets[i].n_packets, &n_packets);
            atomic_read_relaxed(&tx->sub_buckets[i].n_bytes, &n_bytes);
            atomic_init(&new_tx->sub_buckets[i].n_packets, n_packets);
            atomic_init(&new_tx->sub_buckets[i].n_bytes, n_bytes);
        }
                            ...
                            bool should_steal, uint32_t bond)
{
    ...
    DP_PACKET_BATCH_FOR_EACH (i, packet, packets_) {
        /*
         * Lookup the bond-hash table using hash to get the sub-interface.
         */
        uint32_t hash = dp_packet_get_rss_hash(packet);
        struct sub_entry *s_entry = &p_bond->sub_buckets[hash & BOND_MASK];
        odp_port_t bond_member = s_entry->sub_id;
                     ...
                     odp_port_t *sub_map)
{
    ...
    for (int bucket = 0; bucket < BOND_BUCKETS; bucket++) {
        new_tx->sub_buckets[bucket].sub_id = sub_map[bucket];
    }
                           ...
                           uint64_t *n_bytes)
{
    ...
    CMAP_FOR_EACH (pmd, node, &dp->poll_threads) {
        ...
        for (int i = 0; i < BOND_BUCKETS; i++) {
            ...
            atomic_read_relaxed(&pmd_bond_entry->sub_buckets[i].n_bytes,
                                &pmd_n_bytes);
