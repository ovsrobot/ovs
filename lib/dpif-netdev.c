...
tx_bond_lookup(const struct cmap *tx_bonds, uint32_t bond_id)
{
    ...
    pnode = cmap_find(tx_bonds, hash_bond_id(bond_id));
    if (!pnode) {
        return NULL;
    }
    tx = CONTAINER_OF(pnode, struct tx_bond, node);
    return tx;
}
    ...
    OVS_REQUIRES(dp->port_mutex)
{
    ...
    /* Add every bond to the cmap of each pmd thread, if it's not
     * there already and if this pmd has at least one rxq to poll. */
    ovs_mutex_lock(&dp->bond_mutex);
    CMAP_FOR_EACH (pmd, node, &dp->poll_threads) {
    ...
    OVS_REQUIRES(pmd->bond_mutex)
{
    ...
    if (tx) {
        struct tx_bond *new_tx = xmemdup(bond, sizeof *tx);
        ...
        for (int i = 0;i < BOND_BUCKETS; i++) {
            ...
            new_tx->slave_buckets[i].n_packets = n_packets;
            new_tx->slave_buckets[i].n_bytes = n_bytes;
        }
                     ...
                     odp_port_t slave_map[])
{
    ...
    struct tx_bond *new_tx = xzalloc(sizeof(struct tx_bond));
