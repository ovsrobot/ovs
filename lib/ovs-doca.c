...
extern unsigned ofproto_flow_limit; /* ofproto/ofproto.c */
...
/* Warn when ofproto's flow-limit differs from the one set at DOCA
 * initialization. */
void
ovs_doca_check_flow_limit(void)
{
    static unsigned int last_checked = UINT_MAX;
    bool available;
    ...
    if (ofproto_flow_limit == last_checked) {
        return;
    }

    last_checked = ofproto_flow_limit;

    if (ofproto_flow_limit != ovs_doca_max_megaflows_counters) {
        VLOG_WARN_RL(&rl,
                     ...
                     ofproto_flow_limit, ovs_doca_max_megaflows_counters);
    }
}
