...
static struct {
    ...
    const char *doca_level;
} const levels[] = {
    [DOCA_LOG_LEVEL_CRIT]    = { .user_level = "crit", .doca_level = "CRT" },
    [DOCA_LOG_LEVEL_ERROR]   = { .user_level = "err",  .doca_level = "ERR" },
    [DOCA_LOG_LEVEL_WARNING] = { .user_level = "warn", .doca_level = "WRN" },
    [DOCA_LOG_LEVEL_INFO]    = { .user_level = "info", .doca_level = "INF" },
    [DOCA_LOG_LEVEL_DEBUG]   = { .user_level = "dbg",  .doca_level = "DBG" },
    [DOCA_LOG_LEVEL_TRACE]   = { .user_level = "trc",  .doca_level = "TRC" },
};

static int
ovs_doca_parse_log_level(const char *user_level, const char *doca_level)
{
    for (int i = 0; i < ARRAY_SIZE(levels); i++) {
        if (!levels[i].user_level) {
            continue;
        }

        if (user_level && !strncmp(user_level, levels[i].user_level,
            strlen(levels[i].user_level))) {
            return i;
        }

        if (doca_level && !strncmp(doca_level, levels[i].doca_level,
            strlen(levels[i].doca_level))) {
            return i;
        }
    }
...
{
    for (int i = 0; i < ARRAY_SIZE(levels); i++) {
        if (i == log_level && levels[i].user_level) {
            return levels[i].user_level;
        }
    }
                         ...
                         const char *argv[], void *aux OVS_UNUSED)
{
    ...
    if (argc == 2) {
        ...
        level = ovs_doca_parse_log_level(level_string, NULL);
        if (level < 0) {
...
#undef RV_TEST
    ...
    cfg = NULL;
    if (err != DOCA_SUCCESS) {
        VLOG_ERR("DOCA initialization failed, doca_flow_cfg_destroy(). "
                 "Error: %d (%s)", err, doca_error_get_descr(err));
        return ENODEV;
    }
...
static doca_error_t
ovs_doca_check_entry(struct doca_flow_pipe_entry *entry)
{
    enum doca_flow_entry_status status;

    status = doca_flow_pipe_entry_get_status(entry);
    if (status == DOCA_FLOW_ENTRY_STATUS_SUCCESS) {
        return DOCA_SUCCESS;
    } else if (status == DOCA_FLOW_ENTRY_STATUS_IN_PROCESS) {
...
{
    struct ovs_doca_steering_queue *queue;
    ...
    do {
        ...
        if (time_msec() > timeout_ms) {
            static struct vlog_rate_limit timeout_rl =
                VLOG_RATE_LIMIT_INIT(1, 5);

            err = DOCA_ERROR_TIME_OUT;
            VLOG_ERR_RL(&timeout_rl, "Timeout reached trying to complete "
                        "queue %u: %u remaining entries", qid, n_waiting);
        }
    } while (err == DOCA_SUCCESS && room < OVS_DOCA_QUEUE_DEPTH);

    if (entry && err == DOCA_SUCCESS) {
        err = ovs_doca_check_entry(entry);
    }
                     ...
                     struct doca_flow_pipe_entry **pentry)
{
    ...
    struct ovs_doca_steering_queue *queues;
    const struct doca_flow_match *match;
    doca_error_t err;

    ovs_assert(esw && qid < OVS_DOCA_MAX_STEERING_QUEUES);

    queues = esw->steering_queues;
                   ...
                   struct doca_flow_pipe_entry **pentry)
{
    ...
    if (DOCA_FLOW_FLAGS_IS_SET(flags, DOCA_FLOW_ENTRY_FLAGS_NO_WAIT)) {
        err = ovs_doca_complete_queue_esw(esw, qid, *pentry);
    }
...
doca_error_t
ovs_doca_add_entry_hash(struct netdev *netdev,
                        unsigned int qid,
                        struct doca_flow_pipe *pipe,
                        uint32_t index,
                        const struct ovs_doca_flow_actions *actions,
                        ...
                        struct doca_flow_pipe_entry **pentry)
{
    ...
    err = ovs_doca_add_generic(qid, pipe, DOCA_FLOW_PIPE_HASH, index, NULL,
                               actions, monitor, fwd, flags, esw, pentry);
    if (err != DOCA_SUCCESS) {
        VLOG_WARN_RL(&rl, "%s: Failed to create hash pipe entry. "
                     "Error: %d (%s)", netdev_get_name(netdev), err,
                     doca_error_get_descr(err));
        return err;
    }
                      ...
                      struct doca_flow_pipe_entry **pentry)
{
    ...
    ovs_assert(qid < OVS_DOCA_MAX_STEERING_QUEUES);

    err = doca_flow_pipe_remove_entry(qid, flags, *pentry);
    if (err == DOCA_SUCCESS) {
        esw->steering_queues[qid].n_waiting_entries++;
        /* DOCA API is asynchronous.
         *
        ...
        if (DOCA_FLOW_FLAGS_IS_SET(flags, DOCA_FLOW_ENTRY_FLAGS_NO_WAIT)) {
             ...
             * requires knowing so. */
            ovs_doca_complete_queue_esw(esw, qid, NULL);
        }

        *pentry = NULL;
    } else {
        VLOG_ERR("%s: Failed to remove entry for qid=%d. Error: %d (%s)",
                 netdev_get_name(esw->esw_netdev), qid, err,
                 doca_error_get_descr(err));
    }
                               ...
                               uint64_t queues_bitmap)
{
    ...
    for (unsigned int qid = 0; qid < OVS_DOCA_MAX_STEERING_QUEUES; qid++) {
        doca_error_t err;
...
{
    return OVS_DOCA_SLOWPATH_COUNTERS;
}
