                             ...
                             odp_port_t in_port)
{
    ...
    while (iter) {
        ...
        /* TODO only compute len and hash if required? */
    ...
    if(dpcls_key_idx > 0) {
        ...
        for (int i = 0; i < NETDEV_MAX_BURST; i += 8) {
            /* Indexing here is somewhat complicated due to DPCLS output rule load
             * index depending on the hitmask of HWOL/EMC/SMC. More packets from
             * HWOL/EMC/SMC bitmask means less DPCLS rules are used.
             */
             ...
             * There are 8 output pointer per register, subtract the HWOL/SMC/SMC
             * lanes equals the number of DPCLS rules consumed.
             */
