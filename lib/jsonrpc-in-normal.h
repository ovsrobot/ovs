...
static inline struct json *jsonrpc_in_normal_poll(struct jsonrpc_in_normal *nin) {
    size_t n = byteq_tailroom(&nin->input);
    if (n != 0) {
        if (nin->parser == NULL) {
            nin->parser = json_parser_create(0);
        }
        size_t used = json_parser_feed(nin->parser, (char *)byteq_tail(&nin->input), n);
...
jsonrpc_in_normal_wait(struct jsonrpc_in_normal *nin) {
    return byteq_is_empty(&nin->input) ? JSONRPC_IN_IDLE : JSONRPC_IN_ACTIVE_WAKEUP_NOW;
}

static inline size_t
jsonrpc_in_normal_fill_stream_report_data(struct jsonrpc_in_normal *nin, void *data, size_t datasz) {
