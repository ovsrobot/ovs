...
netdev_linux_rxq_construct(struct netdev_rxq *rxq_)
{
    ...
    if (rx->is_tap) {
         ...
         * fragmented UDP.
         */
        static uint32_t last_rcv_sock_buf_size;
        static uint32_t last_snd_sock_buf_size;
        ...
        if (sock_buf_size) {
            ...
            error = setsockopt(rx->fd, SOL_SOCKET, SO_SNDBUF, &sock_buf_size,
                               sock_opt_len);
            if (error && (errno == EBADF || errno == ENOTSOCK)) {
                error = errno;
            ...
            error = setsockopt(rx->fd, SOL_SOCKET, SO_RCVBUF, &sock_buf_size,
                               sock_opt_len);
            if (error && (errno == EBADF || errno == ENOTSOCK)) {
                error = errno;
        ...
        error=  getsockopt(rx->fd, SOL_SOCKET, SO_RCVBUF, &sock_buf_size,
                           &sock_opt_len);
        if (!error) {
            if (last_rcv_sock_buf_size != sock_buf_size) {
                VLOG_INFO("Current socket recv buffer size: %d",
                          sock_buf_size);
                last_rcv_sock_buf_size = sock_buf_size;
            }
        }
        ...
        error = getsockopt(rx->fd, SOL_SOCKET, SO_SNDBUF, &sock_buf_size,
                           &sock_opt_len);
        if (!error) {
            if (last_snd_sock_buf_size != sock_buf_size) {
                VLOG_INFO("Current socket send buffer size: %d",
                          sock_buf_size);
                last_snd_sock_buf_size = sock_buf_size;
            }
        }
    }
    ovs_mutex_unlock(&netdev->mutex);
