    ...
    if (is_ip_any(flow) && !(flow->nw_frag & FLOW_NW_FRAG_LATER)) {
        if (flow->nw_proto == IPPROTO_TCP) {
            ...
            if (is_nd(flow, NULL)
                ...
                if (ntohl(data->igmp_group_ip4) != 0 ||
                    ntohl(data->tcp_flags) != 0) {
                    nd_ext_key =
                         nl_msg_put_unspec_uninit(buf,
                                            OVS_KEY_ATTR_ND_EXTENSIONS,
                                            sizeof *nd_ext_key);
                    nd_ext_key->nd_reserved = ntohl(data->igmp_group_ip4);
        ...
        if (present_attrs & (UINT64_C(1) << OVS_KEY_ATTR_ICMPV6)) {
            ...
            if (is_nd(src_flow, NULL)) {
                ...
                if (present_attrs &
                    ...
                    flow->igmp_group_ip4 = htonl(nd_ext_key->nd_reserved);
                      ...
                      struct ovs_key_nd_extensions *nd_ext)
{
    ...
    nd_ext->nd_reserved = ntohl(flow->igmp_group_ip4);
...
put_nd_extensions_key(const struct ovs_key_nd_extensions *nd_ext,
                      struct flow *flow)
{
    flow->igmp_group_ip4 =htonl(nd_ext->nd_reserved);
                     ...
                     bool use_masked)
{
    ...
    case ETH_TYPE_IPV6:
        ...
        if (ntohs(base->nw_proto == IPPROTO_ICMPV6)) {
