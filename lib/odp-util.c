                       ...
                       int constant)
{
    ...
    case OVS_KEY_ATTR_IPV6_EXTHDRS: {
        const struct ovs_key_ipv6_exthdrs *ipv6_exthdrs_mask = mask;
        return (ipv6_exthdrs_mask->hdrs & IPV6_EXTHDR_MASK)
               == (u16 & IPV6_EXTHDR_MASK);
    }
...
static void
format_ipv6_exthdr(struct ds *ds, const char *name, uint16_t key,
                   const uint16_t *mask, bool verbose)
{
    bool mask_empty = mask && !*mask;

    if (verbose || !mask_empty) {
        bool mask_full = !mask || (*mask & IPV6_EXTHDR_MASK) == IPV6_EXTHDR_MASK;
                      ...
                      bool verbose)
{
    ...
    case OVS_KEY_ATTR_IPV6_EXTHDRS: {
        ...
        format_ipv6_exthdr(ds, "hdrs", key->hdrs, MASK(mask, hdrs), verbose);
        ds_chomp(ds, ',');
        break;
    }
