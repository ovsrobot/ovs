...
#define __OVS_KEY_FIELD(type, name) {offsetof(struct ovs_key_nd_extensions, name), sizeof(type)},
        __OVS_KEY_ND_EXTENSIONS_FIELDS
        {0, 0}
#undef __OVS_KEY_FIELD
    };
    ...
    if (commit(OVS_KEY_ATTR_ND_EXTENSIONS, use_masked, &key, &base, &mask,
               sizeof key, ovs_key_nd_extensions_properties, odp_actions)) {
